...
module.exports = async (req, res) => {
  ...
  const profile = await upsertProfile({
    ...
    plan: kind === "topup" || kind === "rescue" ? existing?.plan || null : data.metadata?.plan || existing?.plan || null,
    trial_used: Number(existing?.trial_used || 0),
    credits_used: Number(existing?.credits_used || 0),
    bonus_credits: Number(existing?.bonus_credits || 0) + ((kind === "topup" || kind === "rescue") ? topupCredits : 0),
